...
test('if pilot extra info updates correctly', () => {
  ...
  newState = fairwayCardReducer(testState, 'modextrainfoen', 'pilotExtraInfo', [], (validationErrors: ValidationType[]) => {}, 'en', 0, '', []);
...
test('if pilot places update correctly', () => {
  ...
  let newState = fairwayCardReducer(testState, newPilotPlace, 'pilotPlaces', [], (validationErrors: ValidationType[]) => {}, 'fi', 0, '', []);
  expect(newState.trafficService?.pilot?.places?.length).toBe(3);

  newState = fairwayCardReducer(testState, '13', 'pilotJourney', [], (validationErrors: ValidationType[]) => {}, 'fi', 1, '', []);
