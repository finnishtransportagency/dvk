...
test('if primary id updates correctly', () => {
  const newState = harbourReducer(testState, 'ModifiedId', 'primaryId', [], (validationErrors: ValidationType[]) => {}, 'fi', '', '', []);
...
test('if reference level updates correctly', () => {
  const newState = harbourReducer(testState, true, 'referenceLevel', [], (validationErrors: ValidationType[]) => {}, 'fi', '', '', []);
...
test('if status updates correctly', () => {
  const newState = harbourReducer(testState, Status.Public, 'status', [], (validationErrors: ValidationType[]) => {}, 'fi', '', '', []);
  expect(newState.status).toEqual(Status.Public);
});

test('if publish info updates correctly', () => {
  const newState = harbourReducer(testState, 'modpublishdetails', 'publishDetails', [], (validationErrors: ValidationType[]) => {}, 'fi', '', '', []);
