...
const GRAVITATIONAL_ACCELERATION: number = 9.80665;
...
export function calculateApparentWindProperties(vesselSpeed: number, vesselCourse: number, windSpeed: number, windDirection: number) {
  ...
  var apparentWindVelocityDrift = Math.sqrt(Math.pow(windSpeed, 2) + Math.pow(vesselSpeedMS, 2) + 2 * windSpeed * vesselSpeedMS * Math.cos(windAngleDriftRAD));
  // Acos((Wind_Speed_Drift_Cal*Cos(Wind_Angle_Rad_Drift_Cal)+Ship_Speed_Drift_Cal)/Apparent_Wind_Velocity_Drift_Cal)
  var apparentWindAngleDrift = toDeg(Math.acos((windSpeed * Math.cos(windAngleDriftRAD) + vesselSpeedMS) / apparentWindVelocityDrift));
...
export function calculateWindForce(airDensity: number, windSpeed: number, windSurface: number, apparentWindAngleDrift: number) {
  ...
  return 0.5 * airDensity * Math.pow(windSpeed, 2) * Math.abs(windSurface) / GRAVITATIONAL_ACCELERATION * DRAG_COEFFICIENT / 1000 * Math.sin(toRad(apparentWindAngleDrift));
}
...
export function calculateEstimatedDriftAngle(lengthBPP: number, draught: number, profileIndex: number, vesselSpeed: number, windSurface: number, airDensity: number, waterDensity: number, apparentWindAngleDrift: number, apparentWindVelocityDrift: number) {
  var driftIndex = 0;
    switch (true) {
    ...
    var driftCoefficient = ((apparentWindAngleDrift - (driftIndex * 10)) * ((DRIFT_COEFFICIENTS[profileIndex][driftIndex+1]
...
export function calculateEstimatedBreadth(lengthBPP: number, breadth: number, estimatedDriftAngle: number) {
  ...
  var estimatedDriftBreadth = lengthBPP * Math.sin(estimatedDriftAngle);
  return (isNaN(estimatedDriftBreadth)? 0 : estimatedDriftBreadth) + parseFloat(breadth.toString());
}

export function calculateHeelDueWind(lengthBPP: number, breadth: number, windSurface: number, displacement: number, GM: number, KB: number, windForce: number) {
  // (Total_Lateral_Surface_Area/Length_BPP/2 + KB) * Wind_Force --> (2*Total_Lateral_Surface_Area/Length_BPP + KB) * Wind_Force
  var windMoment = ((2 * windSurface / lengthBPP) + parseFloat(KB.toString())) * windForce;
  // GM/Wind_Moment/Displacement
  var windHeel = GM / (windMoment / displacement);
  // If(IsBlank(GM), "", Text(Acot(Radians(Wind_Heel)), "0.0"))
  var windHeelDeg = acot(toRad(windHeel));
  return windHeelDeg;
}
...
export function calculateSquatHG(lengthBPP: number, breadth: number, draught: number, blockCoefficient: number, sweptDepth: number,
  waterLevel: number, fairwayFormIndex: number, channelWidth: number, slopeScale: number, slopeHeight: number, vesselSpeed: number, draughtDuringTurn: number) {
  // Ship_m2_HG_Cal | 0.98*Breadth_HG_Cal*Vessel_Draught
  var shipM2 = 0.98 * breadth * draught;
  // Channel_m2_HG_Cal | Channel_Width*Swept_Depth+Swept_Depth*Swept_Depth/Slope_Scale
  var channelM2 = channelWidth * sweptDepth + Math.pow(sweptDepth, 2) / slopeScale;
  // S_HG_Cal | Ship_m2_HG_Cal/Channel_m2_HG_Cal
  var s = shipM2 / channelM2;
  // h1/h_HG_Cal | Slope_Height/Swept_Depth
  var slopeRatio = slopeHeight / sweptDepth;
  ...
  var aCoefficients = [1, 0, 0, 0];
  if (slopeRatio <= 0.3) {
    aCoefficients = [0.46, 15.85, 124.06, -380.04];
  } else if (slopeRatio <= 0.5) {
    aCoefficients = [0.21, 28.20, -53.17, 87.97];
  ...
  var k1 = aCoefficients[0] + aCoefficients[1] * s + aCoefficients[2] * Math.pow(s, 2) + aCoefficients[3] * Math.pow(s, 3);
  ...
  var s1Multiplier = fairwayFormIndex === 0? 0.03 : (fairwayFormIndex === 2? (s / k1) : s);
  // Ks_HG_Cal
  ...
  var squatHG = (2.4 * Ks * blockCoefficient * breadth * draught * Math.pow(froudeNumber, 2)) / (lengthBPP * Math.sqrt(froudeNumber1));
  ...
  var squatHGListed = (2.4 * Ks * blockCoefficient * breadth * draughtDuringTurn * Math.pow(froudeNumber, 2)) / (lengthBPP * Math.sqrt(froudeNumber1));
...
export function calculateUKCStraightCourse(sweptDepth: number, waterLevel: number, correctedDraught: number, squatBarrass: number, squatHG: number) {
  ...
  var resultWOSquat = Number(sweptDepth) + Number(waterLevel) - correctedDraught;
  return [resultWOSquat - squatBarrass, resultWOSquat - squatHG];
}

export function calculateUKCDuringTurn(draught: number, sweptDepth: number, waterLevel: number, correctedDraughtDuringTurn: number, squatBarrass: number, squatHG: number) {
  // (Swept_Depth+Water_Level)-(Draught_During_Turn-Vessel_Draught)*2-Vessel_Draught-Squat
  var resultWOSquat = (Number(sweptDepth) + Number(waterLevel)) - (correctedDraughtDuringTurn - draught) * 2 - draught;
  return [resultWOSquat - squatBarrass, resultWOSquat - squatHG];
}
...
export function calculateWaveLengthProperties(wavePeriod: number, sweptDepth: number) {
  ...
  var waveLengthDeep = (GRAVITATIONAL_ACCELERATION / (Math.PI * 2)) * Math.pow(wavePeriod, 2);
  // k0Shallow_HG_Cal | (Pi()*2)/Wavelength_HG_Cal
  var k0Shallow = (Math.PI * 2) / waveLengthDeep;
  // DShallow_HG_Cal | k0Shallow_HG_Cal*Swept_Depth
  var dShallow = k0Shallow * sweptDepth;
  ...
  var fDShallow = (1 / (1 + (0.6522 * dShallow) + (0.4622 * Math.pow(dShallow, 2)) + (0.0864 * Math.pow(dShallow, 4)) + (0.0675 * Math.pow(dShallow, 5)))) + dShallow;
  // cShallow_HG_Cal | Sqrt((9.80665*Swept_Depth)/fDShallow_HG_Cal)
  var cShallow = Math.sqrt((GRAVITATIONAL_ACCELERATION * sweptDepth) / fDShallow);
  // c0Shallow_HG_Cal | ((Pi()*2)/Wave_Period)/k0Shallow_HG_Cal
  var c0Shallow = ((Math.PI * 2) / wavePeriod) / k0Shallow;
  // WavelengthShallow_HG_Cal | Text((cShallow_HG_Cal/c0Shallow_HG_Cal)*Wavelength_HG_Cal, "[$-fi-FI]0,00")
  var waveLengthShallow = (cShallow / c0Shallow) * waveLengthDeep;
...
export function calculateWaveAmplitudeProperties(lengthBPP: number, waveHeight: number, waveLength: number[]) {
  ...
  var xValue = waveLength[1] / lengthBPP;
  // x_ShallowValue_HG_Cal | WavelengthShallow_HG_Cal/Length_BPP
  var xValueShallow = waveLength[0] / lengthBPP;
  ...
  var RAODeep = xValue > 2.6? 1 : (xValue < 0.65? 0 : (Math.log(xValue) * 0.7061) + 0.3453);
  var RAOShallow = xValueShallow > 2.6? 1 : (xValueShallow < 0.65? 0 : (Math.log(xValueShallow) * 0.7061) + 0.3453);
  // Wavenumber_HG_Cal | Pi()*2/Wavelength_HG_Cal
  // WavenumberShallow_HG_Cal | Pi()*2/WavelengthShallow_HG_Cal
  var waveNumber = Math.PI * 2 / waveLength[1];
  var waveNumberShallow = Math.PI * 2 / waveLength[0];

  // Amplitude_HG_Cal | Text((RAO_HG_Cal*Wavenumber_HG_Cal*Wave_Height/10*Length_BPP)/4, "[$-fi-FI]0,00")
  var waveAmplitudeDeep = (RAODeep * waveNumber * waveHeight * lengthBPP) / 4;
  // AmplitudeShallow_HG_Cal | Text((RAOShallow_HG_Cal*WavenumberShallow_HG_Cal*Wave_Height/10*Length_BPP)/4, "[$-fi-FI]0,00")
  var waveAmplitudeShallow = (RAOShallow * waveNumberShallow * waveHeight * lengthBPP) / 4;
