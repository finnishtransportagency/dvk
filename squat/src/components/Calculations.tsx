...
import { IonText, IonList, IonItem, IonLabel, IonAccordionGroup, IonNote, IonButton, IonAccordion, IonCard, IonCardHeader,
  IonCardTitle, IonCardSubtitle, IonCardContent, IonIcon, IonGrid, IonRow, IonCol } from '@ionic/react';

import { useSquatContext } from '../hooks/squatContext';
import { calculateApparentWindProperties, calculateBowThrusterForce, calculateDraughtDueWind, calculateDraughtDuringTurn, calculateEstimatedBreadth, calculateEstimatedDriftAngle,
  calculateHeelDueWind, calculateHeelDuringTurn, calculateMinimumExternalForce, calculateSafetyMargin, calculateSquatBarrass, calculateSquatHG, calculateUKCDuringTurn,
  calculateUKCStraightCourse, calculateUKCVesselMotions, calculateWaveForce, calculateWindForce, toDeg } from '../utils/calculations';
...
const Calculations: React.FC<ContainerProps> = () => {
  ...
  const isUKCUnderMinimum = () => {
    ...
    if ((showBarrass? squatResult.UKCDuringTurn[0] : squatResult.UKCDuringTurn[1]) < state.environment.attribute.requiredUKC ||
  ...
  useEffect(() => {
    ...
    var [apparentWindVelocityDrift, apparentWindAngleDrift] = calculateApparentWindProperties(
    ...
    // 1.2 Wind Force
    var windForce = calculateWindForce(
    ...
    // 1.3 Wave Force
    var waveForce = calculateWaveForce(
    ...
    var bowThrusterForce = calculateBowThrusterForce(state.vessel.detailed.bowThruster, state.vessel.detailed.bowThrusterEfficiency);

    // 1.5 Remaining Safety Margin
    var remainingSafetyMargin = calculateSafetyMargin(bowThrusterForce, windForce, waveForce);

    // 1.6 Minimum External Force Required
    var minExternalForce = calculateMinimumExternalForce(bowThrusterForce, windForce, waveForce);

    // 3.1 Calculate drift angle for current vessel type
    var estimatedDriftAngle = calculateEstimatedDriftAngle(
      state.vessel.general.lengthBPP,
    ...
    var estimatedBreadth = calculateEstimatedBreadth(state.vessel.general.lengthBPP, state.vessel.general.breadth, estimatedDriftAngle);
  ...
  }, [state.vessel.general.lengthBPP, state.vessel.general.breadth, state.vessel.general.draught,
      state.vessel.detailed.windSurface, state.vessel.detailed.bowThruster, state.vessel.detailed.bowThrusterEfficiency, state.vessel.detailed.profileSelected,
      state.environment.weather.windSpeed, state.environment.weather.windDirection, state.environment.weather.waveHeight,
      state.environment.vessel.vesselSpeed, state.environment.vessel.vesselCourse,
      state.environment.attribute.airDensity, state.environment.attribute.waterDensity]
  );

  // 2. Squat
  useEffect(() => {
    // 2.1 Heel Due Wind
    var heelDueWind = calculateHeelDueWind(
      state.vessel.general.lengthBPP,
    ...
    var constantHeelDuringTurn = calculateHeelDuringTurn(
      state.environment.vessel.vesselSpeed,
    ...
    var correctedDraught = calculateDraughtDueWind(
      state.vessel.general.breadth,
      state.vessel.general.draught,
      heelDueWind
    );
    ...
    var correctedDraughtDuringTurn = calculateDraughtDuringTurn(
      state.vessel.general.breadth,
      state.vessel.general.draught,
    ...
    // 2.8 Squats
    var squatBarrass = calculateSquatBarrass(
      state.vessel.general.draught,
      state.vessel.general.blockCoefficient,
      state.environment.fairway.sweptDepth,
      state.environment.vessel.vesselSpeed
    );
    var [squatHG, squatHGListed] = calculateSquatHG(
      state.vessel.general.lengthBPP,
      state.vessel.general.breadth,
      state.vessel.general.draught,
      state.vessel.general.blockCoefficient,
    ...
    var [UKCStraightCourseBarrass, UKCStraightCourseHG] = calculateUKCStraightCourse(
      state.environment.fairway.sweptDepth,
      state.environment.fairway.waterLevel,
      correctedDraught,
      squatBarrass,
    ...
    // 2.7 UKC During Turn
    var [UKCDuringTurnBarrass, UKCDuringTurnHG] = calculateUKCDuringTurn(
      state.vessel.general.draught,
      state.environment.fairway.sweptDepth,
      state.environment.fairway.waterLevel,
      correctedDraughtDuringTurn,
    ...
    var [UKCVesselMotionBarrass, UKCVesselMotionHG] = calculateUKCVesselMotions(
  ...
  }, [state.vessel.general.lengthBPP, state.vessel.general.breadth, state.vessel.general.draught, state.vessel.general.displacement, state.vessel.general.blockCoefficient,
      state.vessel.detailed.windSurface,
      state.vessel.stability.KG, state.vessel.stability.GM, state.vessel.stability.KB,
      state.environment.weather.waveAmplitude,
      state.environment.fairway.sweptDepth, state.environment.fairway.waterLevel, state.environment.fairway.fairwayForm, state.environment.fairway.channelWidth, state.environment.fairway.slopeScale, state.environment.fairway.slopeHeight,
      state.environment.vessel.vesselSpeed, state.environment.vessel.turningRadius,
      windForceAndDriftResult.windForce]
  );
          ...
          <IonAccordion value="squat">
            ...
            <IonList slot="content">
              <IonItem>
                ...
                <IonNote slot="end"><IonText><h4>{squatResult.heelDueWind.toLocaleString(i18n.language, {maximumFractionDigits: 2})}&deg;</h4></IonText></IonNote>
              </IonItem>
              <IonItem>
                <IonLabel><small>{t("homePage.squat.calculations.constant-heel-during-turn")} (deg)</small></IonLabel>
                <IonNote slot="end"><IonText><h4>{squatResult.constantHeelDuringTurn.toLocaleString(i18n.language, {maximumFractionDigits: 2})}&deg;</h4></IonText></IonNote>
              </IonItem>
              <IonItem>
                <IonLabel><small>{t("homePage.squat.calculations.corrected-draught")} (m)</small></IonLabel>
                <IonNote slot="end"><IonText><h4>{squatResult.correctedDraught.toLocaleString(i18n.language, {maximumFractionDigits: 2})} m</h4></IonText></IonNote>
              </IonItem>
              <IonItem>
                <IonLabel><small>{t("homePage.squat.calculations.corrected-draught-during-turn")} (m)</small></IonLabel>
                <IonNote slot="end"><IonText><h4>{squatResult.correctedDraughtDuringTurn.toLocaleString(i18n.language, {maximumFractionDigits: 2})} m</h4></IonText></IonNote>
              </IonItem>
              <IonItem>
              ...
              <IonItem>
                ...
                <IonNote slot="end"><IonText><h4>{(squatResult.UKCVesselMotions[showBarrass? 0 : 1][0]).toLocaleString(i18n.language, {maximumFractionDigits: 2})} m</h4></IonText></IonNote>
              </IonItem>
              <IonItem>
                <IonLabel><small>{t("homePage.squat.calculations.UKC-straight-course")} (m)</small></IonLabel>
                <IonNote slot="end"><IonText><h4>{(showBarrass? squatResult.UKCStraightCourse[0] : squatResult.UKCStraightCourse[1]).toLocaleString(i18n.language, {maximumFractionDigits: 2})} m</h4></IonText></IonNote>
              </IonItem>
              <IonItem>
                <IonLabel><small>{t("homePage.squat.calculations.UKC-during-turn")} (m)</small></IonLabel>
                <IonNote slot="end"><IonText><h4>{(showBarrass? squatResult.UKCDuringTurn[0] : squatResult.UKCDuringTurn[1]).toLocaleString(i18n.language, {maximumFractionDigits: 2})} m</h4></IonText></IonNote>
              </IonItem>
              {state.environment.fairway.fairwayForm &&
                <IonItem>
                  <div>
                    <IonLabel position="stacked">{t("homePage.squat.calculations.squat")}, {t(state.environment.fairway.fairwayForm.name)} (m)</IonLabel>
                  </div>
                  <IonLabel>{(showBarrass || isUKCUnderMinimum()? squatResult.squatBarrass : squatResult.squatHG).toLocaleString(i18n.language, {maximumFractionDigits: 2})} m</IonLabel>
                  <IonNote slot="end">
                    <IonButton size="small" shape="round" fill="outline" strong={true} title={t("homePage.squat.calculations.show-barrass-value")} onPointerDown={e => toggleSquat(true)} onPointerUp={e => toggleSquat(false)}>
                      ?
                    </IonButton>
                  </IonNote>
                </IonItem>
