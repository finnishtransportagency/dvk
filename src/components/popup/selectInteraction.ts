...
export function addPointerMoveInteraction(map: Map, types: string[]) {
  ...
  const handleSelect = (f: Feature) => {
    ...
    if (['specialarea2', 'specialarea15', 'observation', 'forecast', 'line'].includes(type)) {
    ...
    if (['specialarea2', 'specialarea15', 'observation', 'forecast', 'line'].includes(type)) {
      ...
      if (clickInteraction) {
        clickInteraction.getFeatures().forEach((feat) => {
          ...
          if (['specialarea2', 'specialarea15', 'observation', 'forecast', 'line'].includes(featType) && feat.getId() === f.getId()) {
            found = true;
          }
        });
        if (!found) {
