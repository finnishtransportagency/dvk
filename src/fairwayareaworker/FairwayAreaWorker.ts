...
import { getArea12BorderFeatures } from './FairwayAreaUtils';
...
onmessage = (e: MessageEvent<{ faData: string }>) => {
  ...
  const borderLineFeatures = getArea12BorderFeatures(format.readFeatures(faData, { dataProjection: 'EPSG:4326', featureProjection: MAP.EPSG }));
