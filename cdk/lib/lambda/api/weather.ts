...
export async function fetchMareoGraphs(): Promise<Mareograph[]> {
  ...
  const url = `https://${await getWeatherSOAUrl()}/fmi-apikey/${await getWeatherApiKey()}/timeseries?param=fmisid,geoid,latlon,station_name,localtime,WLEV_PT1S_INSTANT,WLEVN2K_PT1S_INSTANT&precision=double&endtime=now&producer=observations_fmi&timeformat=sql&format=json&keyword=mareografit`;
