...
then
  echo "Environment variable ENVIRONMENT missing. Use setenv.sh script to set it."
    ...
    if [ "$ALB" != "" ]
    then
        aws ssm start-session --target i-0fd8faff073a69865 --document-name AWS-StartPortForwardingSessionToRemoteHost --parameters "host=$ALB,portNumber"=["80"],"localPortNumber"=["$PORT"] > /dev/null &
